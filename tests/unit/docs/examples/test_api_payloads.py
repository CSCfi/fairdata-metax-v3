...
def test_v1_v3_dataset_v3(client, data_catalog, reference_data, v1_v3_dataset_v3_json):
    ...
    assert dataset.access_rights.description == v1_v3_dataset_v3_json["access_rights"]["description"]
    assert dataset.field_of_science.first().url == v1_v3_dataset_v3_json["field_of_science"][0]["url"]
...
def test_v1_v3_data_catalog_v3(client, v1_v3_data_catalog_v3_json, reference_data):
    res = client.post("/v3/data-catalogs", v1_v3_data_catalog_v3_json, content_type="application/json")
    assert res.status_code == 201
