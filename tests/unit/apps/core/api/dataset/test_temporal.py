...
def test_create_temporal_dataset(client, dataset_a_json, data_catalog, reference_data):
    resp = client.post("/v3/datasets", dataset_a_json, content_type="application/json")
    assert resp.status_code == 201
    assert len(resp.json()["temporal"]) == 1


def test_create_temporal_dataset_fail(client, dataset_a_json, data_catalog, reference_data):
