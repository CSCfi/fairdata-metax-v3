...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("description", "Alfa", 1),
        ("accesstype_url", "bargo", 1),
        ("accesstype_url", "suomi.fi", 4),
        ("accesstype_title", "avoin", 1),
        ("accesstype_title", "rajoitettu", 2),
...
@pytest.mark.django_db
def test_list_access_rights_with_filter(client, post_access_rights_payloads, access_right_filter, filter_value, filter_result):
...
@pytest.mark.django_db
def test_change_access_right(client, access_right_alfa_json, access_right_put_alfa_json):
    res1 = client.post(
        "/rest/v3/accessright", access_right_alfa_json, content_type="application/json"
    )
    access_right_created = AccessRight.objects.get(id=res1.data.get('id'))
    assert access_right_created.description['fi'] == "Sisältää aineistoja Alfa-palvelusta"
    response = client.put(
    ...
    assert access_right_changed.description['fi'] == "Sisältää aineistoja MUUTETTU-palvelusta"
...
def test_get_access_right_by_id(client, post_access_rights_payloads):
    ...
    for result in results:
        access_right_by_id = client.get("/rest/v3/accessright/{id}".format(id=result.get("id")))
        assert response.status_code == 200
...
def test_delete_access_right_by_id(client, post_access_rights_payloads):
    ...
    results = response.data.get("results")
    delete_result = client.delete("/rest/v3/accessright/{id}".format(id=results[0].get("id")))
