...
def test_update_dataset(client, dataset_a_json, dataset_b_json, data_catalog, reference_data):
    res = client.post("/rest/v3/dataset", dataset_a_json, content_type="application/json")
    id = res.data["id"]
    res = client.put(f"/rest/v3/dataset/{id}", dataset_b_json, content_type="application/json")
...
def test_create_dataset_invalid_catalog(client, dataset_a_json):
    ...
    response = client.post("/rest/v3/publisher", dataset_a_json, content_type="application/json")
