...
def test_create_datastorage(client, datastorage_a_json):
    res = client.post('/rest/v3/datastorage', datastorage_a_json, content_type='application/json')
...
def test_update_datastorage(client, datastorage_a_json, datastorage_a_updated_json):
    ...
    res = client.put(f'/rest/v3/datastorage/{ds_id}', datastorage_a_updated_json, content_type='application/json')
    assert res.status_code == 200
    assert "This is an updated test data storage" in str(res.data)


@pytest.mark.django_db
def test_update_invalid_datastorage(client, datastorage_a_json, datastorage_a_invalid_json):
    ds_id = datastorage_a_json["id"]
    res = client.post('/rest/v3/datastorage', datastorage_a_json, content_type='application/json')
    assert res.status_code == 201

    res = client.put(f'/rest/v3/datastorage/{ds_id}', datastorage_a_invalid_json, content_type='application/json')
    assert res.status_code == 400
...
@pytest.mark.parametrize("storage_filter, filter_value, filter_result", [
...
@pytest.mark.django_db
def test_filter_datastorage(client,
                           post_datastorage_payloads_a_b_c,
                           storage_filter,
                           filter_value,
                           filter_result):
    res = client.get('/rest/v3/datastorage?{0}={1}'.format(storage_filter, filter_value))
    assert res.status_code == 200
    assert len(res.data) == filter_result


@pytest.mark.django_db
def test_list_datastorages_with_simple_ordering(client,
                           post_datastorage_payloads_a_b_c):
...
@pytest.mark.django_db
def test_list_datastorages_with_complex_ordering(client,
                           post_datastorage_payloads_a_b_c):
