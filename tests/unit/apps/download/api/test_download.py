def test_download_placeholder_endpoints(admin_client):
    ...
    assert get_packages.data["detail"] == "Getting available packages through Metax V3 not implemented."
    ...
    assert request_packages.data["detail"] == "Requesting package generation through Metax V3 not implemented."

    authorize_download = admin_client.post("/v3/download/authorize", content_type="application/json")
    assert authorize_download.status_code == 503
    assert authorize_download.data["detail"] == "Resource authorization through Metax V3 not implemented."
