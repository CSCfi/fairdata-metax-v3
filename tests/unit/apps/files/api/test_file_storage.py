...
def test_delete_filestorage(client, filestorage_a_json, file_storage_list_url):
    ...
    res = client.delete(f"/rest/v3/file-storages/{ds_id}")
    assert res.status_code == 204

    res = client.get(f"/rest/v3/file-storages/{ds_id}?include_removed")
...
def test_get_single_filestorage(client, filestorage_a_json, file_storage_list_url):
    ...
    res = client.get(f"/rest/v3/file-storages/{ds_id}")
    assert res.status_code == 200
...
def test_filter_filestorage(
    ...
    res = client.get("/rest/v3/file-storages?{0}={1}".format(storage_filter, filter_value))
    assert res.status_code == 200
...
def test_list_filestorages_with_simple_ordering(client, post_filestorage_payloads_a_b_c):
    url = "/rest/v3/file-storages?ordering=created"
    res = client.get(url)
    assert res.status_code == 200
...
def test_list_filestorages_with_complex_ordering(client, post_filestorage_payloads_a_b_c):
    url = "/rest/v3/file-storages?ordering=endpoint_description,created"
    res = client.get(url)
    assert res.status_code == 200
